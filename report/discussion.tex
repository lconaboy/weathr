...
standard deviation as an estimate of uncertainty for July in South
Africa or January in East Africa is clearly not appropriate, as they
are skewed by the long tail. For these months the median and IQR do a
much better job of selecting the average and its uncertainty. In
January for South Africa (or July for East Africa) the mean and
standard deviation could be used, but the median is still useful even
when the data appear to be normally distributed and it is much better
to use a statistical measure that is appropriate for all the data,
rather than one that is suited to one part of the dataset but
...
\begin{itemize}
  ...
  \item{indeed, the WTIO SSTAs follow cloud fraction anomalies very
    ...
    month;}
  \item{a possible reason for the supression of cloud coverage in the
